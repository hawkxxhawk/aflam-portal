  ...
  if (chunkCount > 0 && combinedStr) {
    ...
    if (Array.isArray(data.categories)) {
      for (const cat of data.categories) {
        if (!['favorites_folder', 'favorites_folder_2', 'favorites_folder_3'].includes(cat.id)) {
          try {
            ...
            console.warn(`Failed to load ${cat.id}.json`, e);
          }
        }
      }
    }
...
async function initApp() {
  try {
    ...
    if (!categories.some(c => c.id === 'favorites_folder')) {
      categories.push({ id: 'favorites_folder', name: 'المفضلة', isSystem: true, section: 'movies' });
    }
    if (!categories.some(c => c.id === 'favorites_folder_2')) {
      categories.push({ id: 'favorites_folder_2', name: 'مفضلة 2', isSystem: true, section: 'movies' });
    }
    if (!categories.some(c => c.id === 'favorites_folder_3')) {
      categories.push({ id: 'favorites_folder_3', name: 'مفضلة 3', isSystem: true, section: 'movies' });
    }
...
function openTargetedImportModal() {
  document.getElementById('settingsMenu').classList.add('hidden');
  const select = document.getElementById('targetedImportCategory');
  select.innerHTML = '';
  ...
  document.getElementById('targetedImportFile').value = '';
  document.getElementById('targetedImportModal').classList.remove('hidden');
}

function closeTargetedImportModal() {
  document.getElementById('targetedImportModal').classList.add('hidden');
}

function extractShortcutsFromJson(parsed, targetCategoryId = null) {
  let rawArray = [];
  ...
  return rawArray.map(item => {
    let name = item.movies_name || item.series_name || item.title || item.name || "موقع بدون اسم";
    let url = item.movies_href || item.series_href || item.url || item.url2 || item.link || "";
    // Support all common image field names from various JSON schemas
    let image = item.movies_img || item.series_img || item.imageUrl || item.image || item.icon || "";

    return {
      id: item.id || item.movies_id || ('site_' + Math.random().toString(36).substr(2, 9)),
      categoryId: targetCategoryId || item.categoryId || item.movies_category || item.folder || 'general',
      name: name,
      url: url,
      image: image || null,   // stored in 'image' so buildIconImg & card renderers can use it
...
async function executeTargetedImport() {
  const fileInput = document.getElementById('targetedImportFile');
  const select = document.getElementById('targetedImportCategory');
  const targetCategoryId = select.value;

  if (!fileInput.files.length) {
    showToast('⚠️ الرجاء اختيار ملف JSON');
    return;
  }

  showToast('⏳ جاري استيراد المواقع للمجلد...');
  ...
  try {
    ...
    const importedShortcuts = extractShortcutsFromJson(parsed, targetCategoryId);
    if (!importedShortcuts) {
      showToast('⚠️ الملف المحدد غير صالح (لا يحتوي على تركيبة بيانات معروفة)');
      return;
    }

    shortcuts = shortcuts.concat(importedShortcuts);

    migrateCategoryIds();
    mergeDuplicateCategories();
    const dupRemoved = deduplicateShortcuts();
    ...
    const addedNet = importedShortcuts.length - dupRemoved;
    showToast(`✅ تم إضافة ${addedNet} موقع${dupRemoved > 0 ? ` (تم تجاهل ${dupRemoved} مكرر)` : ''}`);
    closeTargetedImportModal();

  } catch (e) {
    showToast('❌ حدث خطأ في قراءة ملف الـ JSON');
    console.warn(e);
  }
}
...
async function importDataFile(event) {
  ...
  try {
    ...
    if (files.length === 1 && files[0].name.endsWith('.zip')) {
      ...
      // Determine if uploading chunks (filenames start with data) or independent JSON files
      const isDataChunks = files.some(f => f.name.startsWith('data') && f.name.endsWith('.json'));

      if (!isDataChunks) {
        // Handle isolated category files import
        let updatedCount = 0;
        for (const file of files) {
          ...
          try {
            ...
            let targetCategoryId = fileName;
            // Map English favorite filenames back to internal system IDs
            if (fileName === 'favourit') targetCategoryId = 'favorites_folder';
            else if (fileName === 'favourit2') targetCategoryId = 'favorites_folder_2';
            else if (fileName === 'favourit3') targetCategoryId = 'favorites_folder_3';

            const importedShortcuts = extractShortcutsFromJson(parsed, targetCategoryId);
            if (importedShortcuts) {
              shortcuts = shortcuts.filter(s => s.categoryId !== targetCategoryId);
              shortcuts = shortcuts.concat(importedShortcuts);
              updatedCount++;
            }
          } catch (e) {
            console.warn('Failed to parse folder JSON', file.name);
          }
        }
        if (updatedCount > 0) {
          saveShortcuts();
          ...
          renderIcons();
          renderWelcomeGrid();
          showToast('✅ تم تحديث بيانات المجلد بنجاح');
          event.target.value = '';
          return;
        }
      }

      // Multiple JSON chunk files
      files.sort((a, b) => {
    ...
    if (files.length === 1 && files[0].name.endsWith('.zip')) {
      ...
      if (Array.isArray(data.categories)) {
        for (const cat of data.categories) {
          if (!['favorites_folder', 'favorites_folder_2', 'favorites_folder_3'].includes(cat.id)) {
            const catFile = zip.file(`database_chunks/${cat.id}.json`) || zip.file(`${cat.id}.json`);
            if (catFile) {
              try {
      ...
      const favFiles = ['favourit.json', 'favourit2.json', 'favourit3.json'];
      for (const fName of favFiles) {
        const fFile = zip.file(`database_chunks/${fName}`) || zip.file(fName);
        if (fFile) {
          try {
...
function renderWelcomeGrid() {
  ...
  if (_currentCategory === 'all') {
    ...
    // Determine the section nature of the active folder
    const activeCatObj = categories.find(c => c.id === _currentCategory) || { id: _currentCategory };
    const sectionType = window.getCategorySection(activeCatObj);
    ...
    pageItems.forEach((s, index) => {
      ...
      if (sectionType === 'movies' || isFavView) {
        // Movie card style
...
function buildIconImg(s, size) {
  ...
  // Fallback: colored bubble without first letter/emoji
  return `<div class="icon-fallback" style="width:${size}px;height:${size}px;border-radius:${size * 0.23}px;background:${s.color || '#333'};${bgStyle}"></div>`;
}
...
// ── Global Browser Mode ────────────────────────────────────────────────
let storedGlobalMode = localStorage.getItem('rm_global_browser_mode');
let globalBrowserMode = storedGlobalMode === null ? true : storedGlobalMode === 'true';
...
// ── Category Management ───────────────────────────────────────────────
// Helper to resolve a category's section (used globally)
window.getCategorySection = function(cat) {
  if (cat.section) return cat.section;
  if (['favorites_folder', 'favorites_folder_2', 'favorites_folder_3'].includes(cat.id)) return 'movies';
  return 'sites';
};
...
function renderCategorySelector() {
  function catSection(c) { return window.getCategorySection(c); }

  // 1. Render top bar dropdown — grouped by section
  ...
  // 2. Render Add/Edit modal dropdown — grouped by section
  const selectCombo = document.getElementById('siteCategory');
  if (selectCombo) {
    const sitesCats = categories.filter(c => catSection(c) === 'sites');
    const moviesCats = categories.filter(c => catSection(c) === 'movies');
    let html = '';
    if (sitesCats.length) {
...
function selectCategory(id) {
  ...
  if (id === 'all') {
    ...
    categoryTitle.textContent = `${label} (${itemCount})`;
...
function renderManageCategoriesList() {
  ...
  function renderGroup(sectionId, sectionLabel, sectionEmoji) {
    const groupCats = categories.filter(c => catSection(c) === sectionId);
    if (groupCats.length === 0) return;

    // Section header row
    const headerLi = document.createElement('li');
    headerLi.style.cssText = 'background:none;border:none;padding:14px 4px 6px;pointer-events:none;';
    headerLi.innerHTML = `<span style="font-size:13px;font-weight:800;color:${sectionId === 'movies' ? '#e50914' : '#4fc3f7'};display:flex;align-items:center;gap:6px;">${sectionEmoji} ${sectionLabel}</span>`;
  ...
  renderGroup('sites', 'قسم المواقع', '🌐');
  renderGroup('movies', 'قسم الأفلام', '🎬');
}
...
function generateCategoryId(name) {
  let originalTrimmed = name.trim();
  // Standardize Arabic characters for matching
  let key = originalTrimmed.toLowerCase().replace(/[إأآا]/g, 'ا').replace(/ة/g, 'ه');

  const map = {
    "عام": "general",
    "المفضله": "favorites_folder",
    "مفضله": "favorites_folder",
    "المفضله 1": "favorites_folder",
    "مفضله 1": "favorites_folder",
    "مفضله 2": "favorites_folder_2",
    "المفضله 2": "favorites_folder_2",
    "مفضله 3": "favorites_folder_3",
    "المفضله 3": "favorites_folder_3",
    "افلام عربيه": "ARABIC MOVIES",
    "مسلسلات": "series",
    "مسلسلات عربيه": "arabic series",
    "مسلسلات اجنبيه": "e series",
    "افلام اجنبيه": "E MOVIES",
  ...
  // Fuzzy matching for Arabic variants
  if (key.includes("عربيه") && (key.includes("افلام") || key.includes("فيلم") || key.includes("movies"))) return "ARABIC MOVIES";
  if ((key.includes("اجنبيه") || key.includes("اجنبي")) && (key.includes("افلام") || key.includes("فيلم") || key.includes("movies"))) return "E MOVIES";
  if (key.includes("++")) return "++ RATED";
  if (key.includes("rated")) return "RATED MOVIES";

  // If english, preserve the exact casing the user typed (allowing plus signs!)
  if (/^[a-zA-Z0-9\s\-_+]+$/.test(originalTrimmed)) return originalTrimmed;

  // Strip bad chars
  let safeKey = originalTrimmed.replace(/[^a-zA-Z0-9\s\-_+]/g, '').trim();
  if (safeKey.length > 0) return safeKey;

  return 'cat_' + Date.now();
}

function migrateCategoryIds() {
  let migrated = false;
  ...
  categories.forEach(cat => {
    let canonId = generateCategoryId(cat.name);

    if (!canonicalMap.has(canonId)) {
      canonicalMap.set(canonId, canonId);

      let oldId = cat.id;
      if (oldId !== canonId) {
        cat.id = canonId;
        shortcuts.forEach(s => {
          if (s.categoryId === oldId) s.categoryId = canonId;
        });
        migrated = true;
      }
      mergedCats.push(cat);
    } else {
      // Absolute duplicate detected! We merge any shortcuts targeting the old ID into the canonical ID
      let oldId = cat.id;
      let finalId = canonId;

      if (oldId !== finalId) {
        shortcuts.forEach(s => {
          if (s.categoryId === oldId) s.categoryId = finalId;
        });
        migrated = true;
      }
    }
  ...
  if (categories.length !== mergedCats.length) migrated = true;
  categories = mergedCats;

  if (migrated) saveShortcuts();
}

// ── Merge categories that share the same name ────────────────────────
function mergeDuplicateCategories() {
  // Build a map: normalised-name → first category with that name
  const nameMap = new Map(); // normName → canonical category object
  const idRemap = new Map(); // oldId  → canonicalId  (for orphan shortcuts)

  const kept = [];
  categories.forEach(cat => {
    const normName = cat.name.trim().toLowerCase();
    if (nameMap.has(normName)) {
      // Duplicate name – remap shortcuts to the canonical folder
      const canonicalId = nameMap.get(normName).id;
      if (cat.id !== canonicalId) idRemap.set(cat.id, canonicalId);
    } else {
      nameMap.set(normName, cat);
      kept.push(cat);
    }
  });
  ...
  if (idRemap.size > 0) {
    shortcuts.forEach(s => {
      if (idRemap.has(s.categoryId)) s.categoryId = idRemap.get(s.categoryId);
    });
    categories = kept;
  }
}

// ── Remove duplicate shortcuts inside each folder ────────────────────
// Deduplication key: URL (trimmed lower-case). When URL is empty/absent
// we fall back to name (trimmed lower-case) so name-only items are still
// deduplicated.
function deduplicateShortcuts() {
  // Group by categoryId
  const seen = new Map(); // categoryId → Set of dedup-keys
  const unique = [];

  shortcuts.forEach(s => {
    const catId = s.categoryId || 'general';
    if (!seen.has(catId)) seen.set(catId, new Set());

    const url = (s.url || '').trim().toLowerCase().replace(/\/+$/, ''); // strip trailing slash
    const name = (s.name || '').trim().toLowerCase();
    const key = url || name; // prefer URL, fall back to name

    if (!key || !seen.get(catId).has(key)) {
      if (key) seen.get(catId).add(key);
      unique.push(s);
    }
  });

  const removed = shortcuts.length - unique.length;
  shortcuts = unique;
  return removed;
}
...
function addCategory() {
  ...
  const secSel = document.getElementById('newCategorySection');
  const name = input.value.trim();
  const section = secSel ? secSel.value : 'sites';
  if (!name) return flashInput('newCategoryName');

  // Block creation of a folder whose name already exists
  const normNew = name.trim().toLowerCase();
  const existing = categories.find(c => c.name.trim().toLowerCase() === normNew);
  if (existing) {
    showToast(`⚠️ يوجد مجلد بنفس الاسم "${existing.name}" بالفعل`);
    return flashInput('newCategoryName');
  }

  const id = generateCategoryId(name);
  categories.push({ id, name, section });
  saveShortcuts();
  renderCategorySelector();
  renderManageCategoriesList();
  input.value = '';
